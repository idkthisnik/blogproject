...
class PostsRatingCrud():
    ...
    def get_post_rating_value(self, post_id: int, user_id: int, session) -> int:
        return session.query(RatingPosts).filter(
    ...
    def get_user_total_posts_rating(self, user_id: int, session) -> int:
        query = [i.Value 
                 for i 
                 in session.query(RatingPosts).filter(RatingPosts.RatingReceiverID==user_id)
        ]
        return sum(query) if query != [] else 0

    @Sessioner.dbconnect
    def get_post_total_rating(self, post_id: int, session) -> int:
        results = session.query(RatingPosts).filter(
                                                RatingPosts.RatedPostID==post_id
                                           ).all()
    ...
    @Sessioner.dbconnect
    def check_post_rating_by_value(self, rate_info: RatePostRequest, session) -> bool:
    ...
    @Sessioner.dbconnect
    def check_post_rating_existence(self, post_id: int, user_id: int, session) -> bool:
        query = session.query(RatingPosts).filter(
                                              RatingPosts.RatedID==user_id,
                                              RatingPosts.RatedPostID==post_id
        )
    ...
    def delete_post_rating(self, rate_info: RatePostRequest, session) -> None:
        rating_to_delete = session.query(
                                      RatingPosts
                                 ).filter(
                                      RatingPosts.RatedID == rate_info.user_id,
                                      RatingPosts.RatedPostID == rate_info.post_id
